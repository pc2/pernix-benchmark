...
def transform_cp2k(output_dir: str, df: pd.DataFrame):
    df[['type', 'width', 'internal_iterations']] = df['name'].str.extract(r'BM_cp2k_([^/]+)/width(\d+)/(\d+)/manual_time')
    ...
    df_decompression_pivot = df_decompression.pivot(index='internal_iterations', columns='width', values='bytes_per_second')
...
def transform_pernix(output_dir: str, name: str, df: pd.DataFrame):
    df[['type', 'optimization', 'memory', 'width', 'blocks']] = df['name'].str.extract(r'BM_([^/]+)_block_([^/]+)_([^/]+)_(\d+)/(\d+)')
