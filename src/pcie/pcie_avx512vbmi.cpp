...
struct Avx512vbmiCompressor { template <std::uint8_t W, typename T> int operator()(const T* i,T s,std::uint8_t* o) const { return pernix::mm512_compress_block_avx512vbmi<W,64>(i,s,o); } };
struct Avx512vbmiDecompressor { template <std::uint8_t W, typename T> int operator()(const std::uint8_t* i,T s,T* o) const { return pernix::mm512_decompress_block_avx512vbmi<W,true,64>(i,s,o); } };
#define REGISTER(W,T,TAG) static void BM_pcie_h2d_##TAG##_##W(benchmark::State& s){pernix_benchmark::pcie::BM_pcie_h2d<W,T,Avx512vbmiCompressor>(s);} static void BM_pcie_d2h_##TAG##_##W(benchmark::State& s){pernix_benchmark::pcie::BM_pcie_d2h<W,T,Avx512vbmiDecompressor>(s);} PERNIX_PCIE_PAYLOADS(BM_pcie_h2d_##TAG##_##W); PERNIX_PCIE_PAYLOADS(BM_pcie_d2h_##TAG##_##W)
