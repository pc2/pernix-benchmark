...
struct Bmi2Compressor { template <std::uint8_t W, typename T> int operator()(const T* i,T s,std::uint8_t* o,std::uint32_t b) const { return pernix::mm256_compress_blocks_bmi2<W,64>(i,s,o,b); } };
struct Bmi2Decompressor { template <std::uint8_t W, typename T> int operator()(const std::uint8_t* i,T s,T* o,std::uint32_t b) const { return pernix::mm256_decompress_blocks_bmi2<W,true,64>(i,s,o,b); } };
#define REGISTER(W,T,TAG) static void BM_pcie_h2d_##TAG##_##W(benchmark::State& s){pernix_benchmark::pcie::BM_pcie_h2d<W,T,Bmi2Compressor>(s);} static void BM_pcie_d2h_##TAG##_##W(benchmark::State& s){pernix_benchmark::pcie::BM_pcie_d2h<W,T,Bmi2Decompressor>(s);} PERNIX_PCIE_PAYLOADS(BM_pcie_h2d_##TAG##_##W,W,T); PERNIX_PCIE_PAYLOADS(BM_pcie_d2h_##TAG##_##W,W,T)
...
WIDTHS(float,bmi2f32); WIDTHS(double,bmi2f64);
