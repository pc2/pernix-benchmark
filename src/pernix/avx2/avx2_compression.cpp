...
public:
    int compress(const ValueT *input, const ValueT scale, uint8_t *output) override {
        return pernix::mm256_compress_block_avx2<BIT_WIDTH, 64>(input, scale, output);
    }
};

#define BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION(N, MEM, TYPE, TAG) \
...
PERNIX_FOR_EACH_BIT_WIDTH(BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION, true, float, avx2f32)
PERNIX_FOR_EACH_BIT_WIDTH(BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION, false, float, avx2f32)
PERNIX_FOR_EACH_BIT_WIDTH(BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION, true, double, avx2f64)
PERNIX_FOR_EACH_BIT_WIDTH(BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION, false, double, avx2f64)

#undef PERNIX_FOR_EACH_BIT_WIDTH
#undef BENCHMARK_COMPRESS_BLOCKS_AVX2_FUNCTION
